...
NfcTagPn532 MifareClassic::read(byte *uid, unsigned int uidLength)
{
    ...
    if (success)
    {
        ...
        if (success)
        {
            if (!decodeTlv(data, messageLength, messageStartIndex)) {
                return NfcTagPn532(uid, uidLength, "ERROR"); // TODO should the error message go in NfcTag?
            }
        }
        else
        {
            Serial.print(F("Error. Failed read block "));Serial.println(currentBlock);
            return NfcTagPn532(uid, uidLength, MIFARE_CLASSIC);
        }
    }
    else
    {
    ...
    return NfcTagPn532(uid, uidLength, MIFARE_CLASSIC, &buffer[messageStartIndex], messageLength);
}

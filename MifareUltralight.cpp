...
NfcTagPn532 MifareUltralight::read(byte * uid, unsigned int uidLength)
{
    if (isUnformatted())
    {
        Serial.println(F("WARNING: Tag is not formatted."));
        return NfcTagPn532(uid, uidLength, NFC_FORUM_TAG_TYPE_2);
    }
    ...
    if (messageLength == 0) { // data is 0x44 0x03 0x00 0xFE
        ...
        return NfcTagPn532(uid, uidLength, NFC_FORUM_TAG_TYPE_2, message);
    }
    ...
    return NfcTagPn532(uid, uidLength, NFC_FORUM_TAG_TYPE_2, ndefMessage);
